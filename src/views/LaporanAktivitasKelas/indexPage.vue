<template>
    <div id="all-products">
        ...
        <h4 style="font-weight:bold;text-align: left; padding-top: 25px; text-decoration: underline;">LAPORAN AKTIVITAS KELAS BULANAN</h4>
        ...
        <div v-if="select.selectedYear != '' && select.selectedMonth != ''">
            <router-link :to="{ name: 'laporanAktivitasKelas.print', params: { year: select.selectedYear, month: select.selectedMonth} }" class="btn btn-outline-primary" style="margin: 25px; text-align: center;">CETAK LAPORAN</router-link>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    //mounted
    onMounted(() => {
    axios.post(`https://backend10610.gofit10603.site/api/bookingPresensiKelas/getLaporanAktivitasKelas`)
    .then(response => {
    ...
    function error(){
        toastr.error('Input Bulan atau Tahun tidak boleh kosong!' , { position:"top-right" });
    }
        return {
            laporans,
            akun,
